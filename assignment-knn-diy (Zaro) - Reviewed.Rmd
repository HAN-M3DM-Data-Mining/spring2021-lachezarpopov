...
By doing some math we can also see that the model produces slightly more false positives (~1.7%) than false negatives (~1.5%). Which type of error is more costly would depend on whether the HVAC systems could be turned on manually. If the system can be turned on manually then the cost of false negatives is relatively small as a person can just turn on the system themself in an occupied room. In this case the cost of the false positives would be higher as the system would be wasting energy on empty rooms. 
